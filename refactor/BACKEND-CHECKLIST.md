...
| routes/*.py lazy `globals().update(vars(_app))` | Must inject the WHOLE app namespace per route fn; a static symbol list fails for names re-exported from core/services (e.g. CAPABILITY_CONCEPT_KEYWORDS, topic_for). Also `from app import *` is illegal inside a function. | ☑ preserved |
...
| 3 core/ extracted | (local, not pushed) | ☑ 63 passed | ☑ 72 rules match | ☑ | Created core/constants.py (TOPIC_KEYWORDS, PATTERN_*, CONCEPT_TAXONOMY*, WAR_STORIES*, BASELINE_RUBRIC*, CONCEPT_NORMALIZATION), core/questions.py (taxonomy_for, war_stories_for, baseline_rubric_for, persona_for, pattern_for, topic_for), core/concepts.py (_normalize_concept, _concept_is_present, _translation_source, _find_translation_sibling). Verbatim moves; app.py re-exports them so call sites unchanged. Stateful fns (is_solved/is_due/schedule_review/recurring_missed_*/_compute_concept_match) intentionally left in app.py — they touch PROGRESS/HISTORY, belong in services/persistence (Phase 4). Added core/__init__.py. |
| 4 services/ extracted | (local, not pushed) | ☑ 63 passed | ☑ 72 rules match | ☑ | Created services/{llm,extraction,execution,grading,persistence}.py. Moved verbatim: chat_content/_call_json_extract; OCR+resume/JD extraction + fallback chains; run_sql_case/run_python_case/get_sample_tables; run_judge/_repair_truncated_json/build_judge_transcript/split_wrap_up_reply/hire_verdict/WHITEBOARD_WRAP_RE; _atomic_json + save_* + current_user_id. Broke app<->service circular import via lazy 'from app import ...' inside functions. Added missing stdlib imports (json/os/re/sqlite3/subprocess/tempfile/BytesIO). Fragile fixes preserved: run_judge max_tokens=4096, topic_for schema-agnostic, _repair_truncated_json. |
| 5 blueprints | (local, not pushed) | ☑ 63 passed | ☑ 72 rules match | ☑ | Moved 71 route funcs verbatim into routes/{pages,auth,documents,analytics,practice,interview}.py as Blueprint (no decorator loss — extraction includes preceding @app.route lines). Each route fn gets a lazy `import app as _app; globals().update({k:v for k,v in vars(_app).items() if not k.startswith('__')})` so the FULL app namespace (incl re-exports from core/services) is available at request time with zero call-site edits. app.py is now a thin entrypoint (1278 lines) that registers the 6 blueprints. Flask `from app import *` is illegal inside functions — that's why globals().update is used. Fragile fixes preserved (unchanged route bodies). |
